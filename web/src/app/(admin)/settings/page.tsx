...
export default function SettingsPage() {
  ...
  // Validation rules below must stay in sync with RetentionConfigUpdate @model_validator
  // in observal-server/schemas/retention.py (mins: days>=7, score>=30, count>=1000).
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div>
                    ...
                    {retentionErrors.data_retention_days ? (
                      <p className="text-[10px] text-red-500 mt-0.5">{retentionErrors.data_retention_days}</p>
                    ) : retentionGlobal > 0 ? (
                  ...
                  <div>
                    ...
                    {retentionErrors.score_retention_days && (
                      <p className="text-[10px] text-red-500 mt-0.5">{retentionErrors.score_retention_days}</p>
                    )}
                  </div>
                  <div>
                    <label className="text-xs text-muted-foreground block mb-1">
                    ...
                    {retentionErrors.max_trace_count && (
                      <p className="text-[10px] text-red-500 mt-0.5">{retentionErrors.max_trace_count}</p>
                    )}
                  </div>
                </div>
                ...
                {hasMinRole(getUserRole(), "super_admin") && (
                  <div className="flex items-center justify-between">
                    {retentionErrors.general ? (
                      <p className="text-[11px] text-red-500">{retentionErrors.general}</p>
                    ) : <span />}
                    <Button
                      size="sm"
            ...
            <div className="bg-card border border-border rounded-lg p-6 max-w-md w-full mx-4 space-y-4">
              ...
              {retentionPreview && (
                <div className="text-xs space-y-1 bg-muted/50 rounded p-2">
                  ...
                  {retentionPreview._note && (
                    <p className="text-[10px] text-muted-foreground italic mt-1">{retentionPreview._note}</p>
                  )}
                </div>
              )}
