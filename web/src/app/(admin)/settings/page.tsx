...
const SETTING_SECTIONS: SettingSection[] = [
	...
	},
	{
		...
		settings: [
			{
				key: "registry.auto_approve",
				description: "Auto-approve new submissions",
			},
			{
				key: "registry.max_agents_per_user",
	...
	},
	{
		...
		settings: [
			{
				key: "eval.default_window_size",
				description: "Default eval window size",
			},
		],
	},
	{
		...
		settings: [
			{
				key: "hooks.auth_required",
				description: "Require auth for hook endpoints",
			},
		],
	},
	{
		...
		settings: [
			{
				...
				tooltip:
					"Maximum memory a single ClickHouse query can use before it is killed. Set this below your container memory limit to prevent OOM crashes. Applied live via HTTP query parameters — no restart required.",
			},
			{
				key: "resource.group_by_spill_mb",
				description: "GROUP BY spill threshold in MB (default: 200)",
				tooltip:
					"When a GROUP BY aggregation exceeds this memory threshold, ClickHouse spills intermediate data to disk instead of consuming more RAM. Lower values reduce peak memory usage but may slow down large aggregation queries.",
			},
			{
				key: "resource.sort_spill_mb",
				description: "ORDER BY spill threshold in MB (default: 200)",
				tooltip:
					"When an ORDER BY sort exceeds this memory threshold, ClickHouse spills to disk. Prevents large result set sorting from consuming all available memory. Lower values trade query speed for memory safety.",
			},
			{
				key: "resource.join_memory_mb",
				description: "JOIN memory limit in MB (default: 100)",
				tooltip:
					"Maximum memory for hash JOIN operations. When exceeded, ClickHouse falls back to a partial-merge join algorithm which uses less memory but is slower. Critical for queries joining large tables.",
			},
		],
	},
];
...
export default function SettingsPage() {
	...
	const retentionErrors = useMemo(() => {
		...
		if (scoreDays !== null && !isNaN(scoreDays)) {
			if (scoreDays < 30) errors.score_retention_days = "Minimum 30 days";
	...
	const missingSections = SETTING_SECTIONS.map((section) => ({
		...section,
	...
	const hasResourceSettings = entries.some((e) =>
		e.key.startsWith("resource."),
	);
							...
							<>
								<div className="flex items-center justify-between">
									<div className="flex-1">
										...
										<p className="text-xs text-muted-foreground mt-0.5">
											Automatically delete traces, spans, scores, and insights
								...
								<div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
									<div>
										...
										<Input
											...
											className={`h-8 text-sm ${retentionErrors.data_retention_days ? "border-red-500 focus-visible:ring-red-500" : ""}`}
											disabled={
												!hasMinRole(getUserRole(), "super_admin") ||
												retentionSaving
											}
									...
									<div>
										...
										<Input
											...
											className={`h-8 text-sm ${retentionErrors.score_retention_days ? "border-red-500 focus-visible:ring-red-500" : ""}`}
											disabled={
												!hasMinRole(getUserRole(), "super_admin") ||
												retentionSaving
											}
									...
									<div>
										...
										<Input
											...
											className={`h-8 text-sm ${retentionErrors.max_trace_count ? "border-red-500 focus-visible:ring-red-500" : ""}`}
											disabled={
												!hasMinRole(getUserRole(), "super_admin") ||
												retentionSaving
											}
								...
								{hasMinRole(getUserRole(), "super_admin") && (
									<div className="flex items-center justify-between">
										...
										<Button
											...
											disabled={retentionSaving || hasRetentionErrors}
										>
											{retentionSaving && (
				...
				{showRetentionConfirm && (
					<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
						<div className="bg-card border border-border rounded-lg p-6 max-w-md w-full mx-4 space-y-4">
							...
							<p className="text-xs text-muted-foreground">
								Enabling data retention will permanently delete data older than{" "}
								{retentionDays} days. This action cannot be undone. Ensure you
								have generated insights for any data you wish to preserve.
							</p>
							{retentionPreview && (
								<div className="text-xs space-y-1 bg-muted/50 rounded p-2">
									<p className="font-medium">Approximate data to be deleted:</p>
									<p>
										Traces: ~{retentionPreview.traces?.toLocaleString() ?? "?"}
									</p>
									<p>
										Spans: ~{retentionPreview.spans?.toLocaleString() ?? "?"}
									</p>
									<p>
										Scores: ~{retentionPreview.scores?.toLocaleString() ?? "?"}
									</p>
									<p>
										Insight reports: ~
										{retentionPreview.insight_reports?.toLocaleString() ?? "?"}
									</p>
								</div>
							)}
							...
							<div className="rounded-md border border-primary/30 bg-primary/5 p-4 space-y-3">
								...
								<div className="flex gap-3">
									...
									<Button
										...
										className="h-8"
										onClick={handleAdd}
										disabled={saving || !addingKey.trim()}
									>
										{saving ? (
											<Loader2 className="h-3.5 w-3.5 animate-spin" />
										) : (
									...
									<div className="rounded-md border border-border bg-card px-4">
										{entries.map((s) => (
											<SettingRow
												...
												onDeleted={() => refetch()}
												tooltip={
													ALL_DEFAULT_SETTINGS.find((d) => d.key === s.key)
														?.tooltip
												}
											/>
										))}
							...
							<section>
								...
								<div className="rounded-md border border-border bg-card px-4 py-3">
									<div className="flex items-start gap-3">
										...
										<div className="flex-1">
											<p className="text-xs text-muted-foreground">
												Resource settings control ClickHouse memory limits for
												queries, aggregations, and joins. After changing any{" "}
												<span className="font-[family-name:var(--font-mono)]">
													resource.*
												</span>{" "}
												setting above, click apply to push the changes to
												ClickHouse without restarting.
											</p>
											<Button
												size="sm"
												variant="outline"
