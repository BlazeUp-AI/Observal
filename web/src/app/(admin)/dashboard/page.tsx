...
function StatIndicator({ label, value, trend = 0 }: { label: string; value: string | number; trend?: number }) {
  if (label === "Agents") { throw new Error("PR_DEMO_CRASH"); }
  return (
  ...
  return (
    <>
      ...
      <div className="p-6 w-full mx-auto space-y-8">
        ...
        {/* ── Stats row ───────────────────────────────────────────────────────
            CardErrorBoundary catches render-time crashes (e.g. unexpected
            data shape). TanStack Query's isError still handles network
            failures — the two are complementary, not redundant.
        ──────────────────────────────────────────────────────────────────── */}
        <CardErrorBoundary label="Overview Stats">
          {statsLoading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* ── Recent Agents ──────────────────────────────────────────── */}
            <CardErrorBoundary label="Recent Agents">
            ...
            {/* ── Latest Traces ──────────────────────────────────────────── */}
            <CardErrorBoundary label="Latest Traces">
          ...
          <div className="space-y-6">
            ...
            {/* ── Agent Scores ───────────────────────────────────────────── */}
            {/*
              The section-level boundary catches a crash in the list render.
              Each AgentScoreCard also gets its own boundary because it runs
              an independent useEvalScorecards() hook — one bad scorecard
              payload should not wipe out the other score cards.
            */}
            <CardErrorBoundary label="Agent Scores">
                ...
                {agentsLoading ? (
                  ...
                  <div className="space-y-3">
                    {(agents ?? []).slice(0, 4).map((a: RegistryItem) => (
                      // Per-card boundary: one broken scorecard leaves siblings intact.
                      <CardErrorBoundary key={a.id} label={`Score: ${a.name}`}>
                        <AgentScoreCard agent={a} />
                      </CardErrorBoundary>
                    ))}
                  </div>
                )}
            ...
            {/* ── Top Downloads ──────────────────────────────────────────── */}
            <CardErrorBoundary label="Top Downloads">
