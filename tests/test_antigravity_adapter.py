...
def test_antigravity_registry_required_keys():
    ...
    required = {
        "display_name", "features", "scopes", "default_scope", "rules_file",
        "mcp_config_path", "mcp_servers_key", "hook_config_path", "hook_events_map",
    }
...
def test_detect_hooks_installed(tmp_path):
    hooks = {"observal-telemetry": {"PreInvocation": [{"hooks": [{"type": "command", "command": "wsl.exe python3 -m observal_cli.hooks.antigravity_session_push"}]}]}}
