...
from models.agent import Agent, AgentVisibility, AgentStatus, AgentVersion, AgentTeamAccess
...
async def test_private_agents_not_leaked_to_org_members(db_session):
    ...
    v1 = AgentVersion(agent_id=agent_public.id, version="1.0.0", status=AgentStatus.approved, released_by=owner_id, model_name="test")
    v2 = AgentVersion(agent_id=agent_private.id, version="1.0.0", status=AgentStatus.approved, released_by=owner_id, model_name="test")
    v3 = AgentVersion(agent_id=agent_private_with_group.id, version="1.0.0", status=AgentStatus.approved, released_by=owner_id, model_name="test")
