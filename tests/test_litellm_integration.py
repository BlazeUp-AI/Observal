...
def _reset_warning_state():
    ...
    mod._warned_legacy_key = False
    mod._warned_legacy_url = False
    yield
    mod._warned_model_format.clear()
    mod._warned_legacy_key = False
    mod._warned_legacy_url = False
...
class TestCallLitellm:
    ...
    @pytest.mark.asyncio
    async def test_bedrock_passes_aws_credentials(self, ds_settings):
        """Bedrock models pass AWS credentials to litellm."""
        settings, mock_get = ds_settings
        settings["insights.aws_region"] = "us-west-2"
        settings["insights.aws_access_key_id"] = "AKIATEST"
        settings["insights.aws_secret_access_key"] = "secret123"
        settings["insights.aws_session_token"] = "token456"
    ...
    @pytest.mark.asyncio
    async def test_non_bedrock_does_not_pass_aws_credentials(self, ds_settings):
        """Non-bedrock models don't get AWS credentials."""
        settings, mock_get = ds_settings
        settings["insights.api_key"] = "sk-test"
        settings["insights.aws_access_key_id"] = "AKIATEST"
        settings["insights.aws_secret_access_key"] = "secret123"
        ...
        with (
            ...
            await _call_litellm("test", "anthropic/claude-sonnet-4-20250514")
    ...
    @pytest.mark.asyncio
    async def test_legacy_key_fallback(self, ds_settings):
        """Falls back to legacy model_api_key if api_key is empty."""
        settings, mock_get = ds_settings
        settings["insights.model_api_key"] = "legacy-key"
        settings["insights.model_url"] = "http://legacy-url:11434/v1"
        ...
        with (
            ...
            await _call_litellm("test", "openai/gpt-4o")

        call_kwargs = mock_acompletion.call_args.kwargs
        assert call_kwargs["api_key"] == "legacy-key"
        assert call_kwargs["api_base"] == "http://legacy-url:11434/v1"
...
class TestCallModel:
    ...
    @pytest.mark.asyncio
    async def test_model_override_normalized(self):
        """Legacy model_override is normalized before calling LiteLLM."""
        ...
        with (
            ...
            await call_model("test", model_override="us.anthropic.claude-opus-4-6-v1")
