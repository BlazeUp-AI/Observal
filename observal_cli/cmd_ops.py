...
_EVAL_SEED_INGEST_DELAY_ENV = "OBSERVAL_SEED_INGEST_DELAY_MS"
...
def _eval_seed_ingest_delay_seconds() -> float:
    raw = os.environ.get(_EVAL_SEED_INGEST_DELAY_ENV, "500")
    try:
        delay_ms = max(0, int(raw))
    except ValueError:
        rprint(f"[yellow]Invalid {_EVAL_SEED_INGEST_DELAY_ENV}={raw!r}; using 500ms.[/yellow]")
        delay_ms = 500
...
def eval_seed(
    ...
    delay = _eval_seed_ingest_delay_seconds()
    if delay:
        time.sleep(delay)
    for trace_id in trace_ids:
